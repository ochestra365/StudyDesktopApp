...
/*
 상당히 많은 오류가 발생하고 있따. 컨트롤 박스의 네임설정부터 초기 프로젝트의 복사 붙여넣기가 순차적으로 이루어지지 않았다.
프로그래밍을 할 수록 오류가 누적되는 경험을 할 수 있었다.
고치기 위해서는 컨트롤 네임설정을 데이터베이스 컬럼명과 똑같이 설정하고, 반복되는 로직을 메서드 처리하는 것과
region, endregion으로 묶어서 분류를 잘해야 한다.
 */
